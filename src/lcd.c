...
void LCDInit(void)
{
	...
	Lcd_WritePort (0x00);	//Display OFF Cursor OFF Parpadeo OFF
	LCDSend();
	Lcd_Delay(60);
	LCDClearData;
	Lcd_WritePort (0x08);
	...
	LCDClearData;
	Lcd_WritePort (0x00);	//Reseteo Cursor y DDRAM
	LCDSend();
	Lcd_Delay(60);
	LCDClearData;
	Lcd_WritePort (0x01);
	...
	LCDClearData;
	Lcd_WritePort (0x00);	//Movimiento del cursor y desplazamiento
	LCDSend();
	Lcd_Delay(60);
	LCDClearData;
	Lcd_WritePort (0x06);
...
#ifdef PORT_INVERTED
	LCDClearData;
	data_w = SwapNibble(data_w);
	lcdPort2->BSRR |= (data_w << lcdDataPins);
#else
	LCDClearData;
	lcdPort2->BSRR |= (data_w << lcdDataPins);
#endif
